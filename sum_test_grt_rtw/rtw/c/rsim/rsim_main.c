...
/* Parameters for calculating stiffness when variable step auto solver is used */
...
static void rsimOneStep(SimStruct *S)
{
    ...
    /* Call to finalize selection of solver when the compiled solver is an 
       variable step auto solver. The solver is selected based on 
       1. Stiffness Parameters - Possible solvers are ode45 or ode15s
       2. Only Discrete States - No continuous state implies VariableStepDiscrete solver
       3. System is linearly implicit/DAE - Solver chosen is ode23t */
    rsimFinalizeAutoSolverForStiffModelsIfNeeded(S, gblSlvrJacPatternFileName,
...
static void rsimOneStep(SimStruct *S)
{
    ...
    if (ssGetSampleTime(S,0) == CONTINUOUS_SAMPLE_TIME) { 
        ...
        SLSize nx = ssGetNumContStates(S);
...
int_T main(int_T argc, char_T *argv[])
{
    ...
    /* Initialize i18n services explicitly */
    i18n_init();
...
/*Enable logging in the MdlStart method*/
    ...
    result = rt_RSimReadInportsMatFile(gblInportFileName, &matFileFormat);
    if (result!= NULL) {
